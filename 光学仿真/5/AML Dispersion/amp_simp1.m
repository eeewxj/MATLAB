function [Eout,gain] = amp_simp1(Ein,GssdB,PoutsatdB,NF,num)
% amp_simp(Ein,GssdB,PoutsatdB,NF,num)
...
G = G - step;
gain(ii) = G;
Egain = sqrt(G)*Ein(n*(ii-1)+1:n*ii);

Pase = (FigNoise*G-1)*h_plan*Fcar/2*BW;
PasedB = 10*log10(Pase);

Eout(n*(ii-1)+1:n*ii) = Egain + wgn(n,1,PasedB,'complex');
