...
if (length(alphaa) == nt)     % If the user manually specifies alpha(w)
  ...
  for ii = 0:length(alphaa)-1;
    ha = ha + alphaa(ii+1)*(w).^ii/factorial(ii);%modified by wxj
  end
  ha = ha/2;
end

if (length(betapa) == nt)     % If the user manually specifies beta(w)
  ha = ha + 1i*betapa;                              %modified by wxj
else
  for ii = 0:length(betapa)-1;
    ha = ha + 1i*betapa(ii+1)*(w).^ii/factorial(ii);%modified by wxj
  end
end
...
if (length(alphab) == nt)
  ...
  for ii = 0:length(alphab)-1;
    hb = hb + alphab(ii+1)*(w).^ii/factorial(ii);%modified by wxj
  end
  hb = hb/2;
end

if (length(betapb) == nt)
  hb = hb + 1i*betapb;                               %modified by wxj
else
  for ii = 0:length(betapb)-1;
    hb = hb + 1i*betapb(ii+1)*(w).^ii/factorial(ii);%modified by wxj
  end
end
...
if strcmp(method,'circular')   %% CIRCULAR BASIS METHOD %%
  ...
  for iz = 1:nz,
    ...
    uahalf = fft( h11.*uafft + h12.*ubfft )/nt;
    ubhalf = fft( h21.*uafft + h22.*ubfft )/nt;
    for ii = 1:maxiter,
      % Calculate nonlinear section
      uva = uahalf .* exp( (1i*(1/3)*gamma*dz).* ...%modified by wxj
                           ( 2*(abs(u0a).^2+abs(u1a).^2)/2 + ...
                             4*(abs(u0b).^2+abs(u1b).^2)/2 ) );
      uvb = ubhalf .* exp( (1i*(1/3)*gamma*dz).* ...%modified by wxj
...
elseif strcmp(method,'elliptical')    %% ELLIPTICAL BASIS METHOD %%
  ...
  u0a = ( cos(psi)*cos(chi) + 1i*sin(psi)*sin(chi))*u0x + ...
        ( sin(psi)*cos(chi) - 1i*cos(psi)*sin(chi))*u0y;
  u0b = (-sin(psi)*cos(chi) - 1i*cos(psi)*sin(chi))*u0x + ...
        ( cos(psi)*cos(chi) - 1i*sin(psi)*sin(chi))*u0y;
  ...
  for iz = 1:nz,
    % Calculate 1st linear half
    uahalf = fft( ha.*uafft )/nt;
    ubhalf = fft( hb.*ubfft )/nt;
    for ii = 1:maxiter,
      % Calculate nonlinear section
      uva = uahalf .* exp( (1i*(1/3)*gamma*dz).* ...%modified by wxj
  ...
  u1x = ( cos(psi)*cos(chi) - 1i*sin(psi)*sin(chi))*u1a + ...
        (-sin(psi)*cos(chi) + 1i*cos(psi)*sin(chi))*u1b;
  u1y = ( sin(psi)*cos(chi) + 1i*cos(psi)*sin(chi))*u1a + ...
        ( cos(psi)*cos(chi) + 1i*sin(psi)*sin(chi))*u1b;
