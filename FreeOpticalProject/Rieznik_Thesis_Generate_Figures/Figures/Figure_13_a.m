...
lamdazero = 1550.0;                     % nm
pump_power_per_pump_per_mode = 1;       % W
...
u0x = upump*exp(-j*2*pi*1e-3*v(aux1b)*t) + upump*exp(-j*2*pi*1e-3*v(aux2b)*t);
u0y = 0;
...
for ii = 1:gainpoints/2;
    u0y = u0y + sqrt(input_signal_power).*exp(j*2*pi*1e-3*vs(nt/2 + stepfreq*ii)*t);
    u0y = u0y + sqrt(input_signal_power).*exp(j*2*pi*1e-3*vs(nt/2 - stepfreq*ii + floor(stepfreq/2))*t);
end
...
P1= pump_power_per_pump_per_mode;
P2 = pump_power_per_pump_per_mode;
P0 = P1+P2;
L = nz*dz;
u = -5/3;
b = 1/3;
