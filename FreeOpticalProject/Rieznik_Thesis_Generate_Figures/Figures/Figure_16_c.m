...
format long e
fprintf(1,'\n----------------------------------------------');
fprintf(1,'\nSimulating Figure 16c of Rieznik´s thesis'); 
% CONSTANTS
...
dt = 1e-2;                              % time step (ps)
dz = 0.005;                                 % distance stepsize (m)
...
betapx = [0 beta1x beta2x beta3x beta4x]';	    % polynomial beta coefs in x
betapy = [0 beta1y beta2y beta3y beta4y]';	    % polynomial beta coefs in y
...
load tetas400B
load beta0s400B
tetas = interp1(0:0.5:199.5,tetas,0:dz:nz*dz); 
beta0s = interp1(0:0.5:199.5,beta0s,0:dz:nz*dz); 
...
createFig6c(vw,10*log10(selectedgains),10*log10(selectedgains2));
