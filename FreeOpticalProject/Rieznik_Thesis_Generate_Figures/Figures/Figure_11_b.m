...
u0x = upump* exp(j*(pi/2))*exp(-j*2*pi*1e-3*v(aux1b)*t) + upump*exp(-j*2*pi*1e-3*v(aux2b)*t);
u0y = upump*exp(-j*2*pi*1e-3*v(aux1b)*t) + exp(j*(pi/2))*upump*exp(-j*2*pi*1e-3*v(aux2b)*t);
